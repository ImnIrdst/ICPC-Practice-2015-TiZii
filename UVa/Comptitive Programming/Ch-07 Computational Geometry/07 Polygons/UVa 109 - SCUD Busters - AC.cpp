...
int main(){
	int n; Point p; vector< vector<Point> > polys;
	while(cin >> n && n!=-1){
		vector<Point> pts, CH;
		for(int i=0 ; i<n ; i++){
			cin >> p.x >> p.y; pts.push_back(p);
		}
		CH = convex_hull(pts); polys.push_back(CH);
	}
	n = polys.size(); vector<bool> attacked(n,false);
	while(cin >> p.x >> p.y){
		for(int i=0 ; i<n ; i++){
			if(attacked[i]) continue;
			if(PointInPolygon(polys[i],p)){
				attacked[i]=true; break;
			}
		}
	}
	double ans = 0;
	for(int i=0 ; i<n ; i++){
		if(attacked[i]) ans += Area(polys[i]);
	}
	printf("%.2lf\n", ans);
}
