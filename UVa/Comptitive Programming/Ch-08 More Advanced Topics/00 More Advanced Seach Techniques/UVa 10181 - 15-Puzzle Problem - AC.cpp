...
// int    dx[]={  0,  0,  1, -1};
// int    dy[]={ -1, +1,  0,  0};
// char parr[]={'L','R','D','U'}, path[60];

// struct state{
//     int a[4][4]; int g,h; int64 key; string par;
//     int* operator[](int   i) {return a[i];}
//     state operator = (state& s){
//         g=s.g; h=s.h; par=s.par;
//         for(int i=0 ; i<4 ; i++) 
//             for(int j=0 ; j<4 ; j++)
//                a[i][j]=s[i][j];
//         return s;
...
//         for(int i=0 ; i<4 ; i++){
//             for(int j=0 ; j<4 ; j++){
//                 if(a[i][j]==0) zi=i, zj=j;
//             }
//         }
//         state tmp, Null; tmp=*this; Null[0][0]=-1;
//         if(!inRange(zi+dx[w],zj+dy[w])) return Null;//TODO
//         swap(tmp[zi][zj],tmp[zi+dx[w]][zj+dy[w]]);
//         tmp.init(g+1);
//         return tmp;
//     }
// };

// string ans; int lim; set<int64> st; int itrs;

// typedef pair<int,char> pic;

// int DFS(state s){
//     stack<pic> st2; st2.push(pic(0,'0'));
//     stack<state> stk; stk.push(s); int ret=1e9; 
//     while(!stk.empty()){
//         s = stk.top(); stk.pop(); pic ic=st2.top(); st2.pop(); s.init(ic.first); 
//         if(s.g+s.h>45 || s[0][0]==-1) continue;
//         path[ic.first]=ic.second;
//         if(st.count(s.key)) continue; st.insert(s.key);
//         if(s.h == 0){path[ic.first+1]='\0'; ans=path; break;}
//         if(s.g+s.h>lim){ ret=min(ret,s.g+s.h); continue;}
//         itrs++;
//         for(int i=0 ; i<4 ; i++){
//             state tmp = s.next(i);
//             if(tmp[0][0]!=-1) stk.push(tmp), st2.push(pic(ic.first+1,parr[i]));
//         }
//     }
//     return ret;
// }
...
// void IDA(state& s){
//     lim=0;
//     while(lim!=1e9 && lim<=45 && ans.size()==0){
//        st.clear(); lim=DFS(s); //cout << itrs << " " << lim << endl;
//     }
...
//         }
//         s.init(0); IDA(s);
//         if(ans.size()==0) cout << "This puzzle is not solvable." << endl;
//         if(ans.size()!=0) cout << ans.substr(1) << endl;
...
{  
    ...
    {  
        ...
        // 将状态特征值和状态评分存入闭合集中。  
        closed.insert(pair< unsigned long long, int >  
            ((unsigned long long)key(current.state), current.score));  
