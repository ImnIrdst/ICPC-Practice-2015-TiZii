...
void LCP_Calc(){
	...
	for(int i=L=0 ; i<n ; i++){
		...
		while(str[i+L]==str[Phi[i]+L] && L<n && str[i+L]!='.')
            L++;
...
bool check(int idx,int Max){//if Consecutive idxes Blongs to two difrent Strings
    st2.clear(); st2.insert(par[SA[idx]]);
	for(int i=idx, cnt=1 ; i>=0 ; i--){
		if(!st2.count(par[SA[i-1]]) && LCP[i]==LCP[idx] && (str+SA[idx])[0]==(str+SA[i])[0]){
			cnt++; st2.insert(par[SA[i-1]]);
			if(cnt>N/2) return 1;
		}
	}
	return 0;
